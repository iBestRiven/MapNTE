...
def pick_area_id(row: dict[str, Any]) -> str | None:
    """
    这个 DT 里的 AreaId 通常是 'None'，
    实际区域更像是 BelongsGameplayAreaID。
    """
    for key in ("BelongsGameplayAreaID", "AreaId"):
        value = row.get(key)
...
def clean_rows(rows: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for fallback_index, (name, row) in enumerate(rows.items()):
        if not isinstance(row, dict):
            continue

        location = row.get("OverrideLocation")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="清洗 DT_QuestFixedMapIconData_4N，仅保留 name/index/AreaId/location"
    )
    parser.add_argument("input", help="输入 JSON 文件")
    parser.add_argument(
        "-o",
        "--output",
        default="cleaned_quest_fixed_map_icon.json",
        help="输出 JSON 文件",
    )
